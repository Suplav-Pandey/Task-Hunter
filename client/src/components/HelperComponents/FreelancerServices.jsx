...
export default function FreelancerServices() {
  ...
  useEffect(() => {
    tokenExists(token, navigate, dispatch).then(data => (data == false || (JSON.parse(localStorage.getItem('userInfo')).role != "helper" && JSON.parse(localStorage.getItem('userInfo')).role != "freelancer") || JSON.parse(localStorage.getItem('userInfo'))._id != id) && navigate("/login"))
  }, [])
        ...
        <div className="container">
          <div className="section">
            <div className="buttons">
              <HashLink to={`/dashboard/helper/${id}/jobs/create`}><button>Create Job</button></HashLink>
              <HashLink to={`/dashboard/helper/${id}/jobs/manage`}><button>Manage Jobs</button></HashLink>
            </div>
