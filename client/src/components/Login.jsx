    ...
    useEffect(() => {
        tokenExists(token, navigate, dispatch).then(() => {
            if (localStorage.getItem('userInfo')) {
                ...
                } else if (connectedUser.role == "freelancer" || connectedUser.role == "helper") {
                    navigate(`/dashboard/helper/${connectedUser._id}`)
                }
            }
        })
    }, [])
            ...
            dispatch(login(body)).unwrap().then(data => {
                setTimeout(() => {
                    ...
                    if (data.status == 200) {
                        ...
                        } else if (data.userInfo.role == "freelancer" || data.userInfo.role == "helper") {
                            navigate(`/dashboard/helper/${data.userInfo._id}`)
                        }
                    }
                    else {
